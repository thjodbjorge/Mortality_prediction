...
#     X.loc[no_bmi_ind,'bmi'] = X.loc[I_train].bmi.mean()
    ...
    print(X_train[tradextralog].isna().sum())
...
#             file = open(folder+feat_folder+"{}_{}_features_security.pkl".format(endpoint,dataset),'rb')
#             features_dict = pickle.load(file)  
...
#         proteins = ['SeqId.4374-45', 'SeqId.11388-75','SeqId.3339-33','SeqId.15559-5','SeqId.13565-2','SeqId.4153-11','SeqId.2654-19','SeqId.2602-2','SeqId.4496-60','SeqId.8099-42']
#         protein_names = ['GDF15','HE4','TSP2','ANTR2','p130','A1AC','TNFsR1','Angp2','MMP12','SPON2']
        proteins = ['SeqId.2652-15']
        protein_names = ['PLAUR']
...
#                 feat.extend(agesex)
    ...
    #             feat.extend(cumunibor15)
                pred_dict['{}_y{}_{}_lr'.format(dataset,k,protein_names[i])] = pf.predict(feat=feat,X_train=X_train,y_train=y_train,X_test=X_test,y_test=y_test,model_type='lr')
                f = open(folder+pred_folder+"{}_{}_predict_single_protein.pkl".format(endpoint,dataset),"wb")
            ...
            try:
                ...
                feat.extend([pro])
    #             feat.extend(cumunibor15)
                pred_dict['{}_y{}_agesex{}_lr'.format(dataset,k,protein_names[i])] = pf.predict(feat=feat,X_train=X_train,y_train=y_train,X_test=X_test,y_test=y_test,model_type='lr')
                f = open(folder+pred_folder+"{}_{}_predict_single_protein.pkl".format(endpoint,dataset),"wb")
                pickle.dump(pred_dict,f)
                f.close()
                print('Done k = ',k)
...
#                 feat.extend([pro])
#                 pred_dict['{}_y{}_baseline{}_lr'.format(dataset,k,protein_names[i])] = pf.predict(feat=feat,X_train=X_train,y_train=y_train,X_test=X_test,y_test=y_test,model_type='lr')
#                 f = open(folder+pred_folder+"{}_{}_predict_single_protein.pkl".format(endpoint,dataset),"wb")
#                 pickle.dump(pred_dict,f)
#                 f.close()
#                 print('Done k = ',k)
...
#                 print(e)
            try:
                ...
                feat.extend(trad2)
                feat.extend([pro])
                pred_dict['{}_y{}_baseline2{}_lr'.format(dataset,k,protein_names[i])] = pf.predict(feat=feat,X_train=X_train,y_train=y_train,X_test=X_test,y_test=y_test,model_type='lr')
                f = open(folder+pred_folder+"{}_{}_predict_single_protein.pkl".format(endpoint,dataset),"wb")
                pickle.dump(pred_dict,f)
                f.close()
                print('Done k = ',k)
